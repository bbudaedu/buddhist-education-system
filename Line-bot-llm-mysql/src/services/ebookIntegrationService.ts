...
export class EbookIntegrationService extends EventEmitter {
    ...
    return rawBooks.map((book, index) => {
      try {
        ...
        const bookSummary: BookSummary = {
          ...
          downloadUrl: this.addExternalBrowserParam(book.pdf_url || book.downloadUrl || ''),
  ...
  /**
   * 為 PDF URL 添加外部瀏覽器參數
   * @param url 原始 PDF URL
   * @returns 添加了 openExternalBrowser=1 參數的 URL
   */
  private addExternalBrowserParam(url: string): string {
    if (!url) return url;
    
    try {
      const urlObj = new URL(url);
      
      // 檢查是否已經有 openExternalBrowser 參數
