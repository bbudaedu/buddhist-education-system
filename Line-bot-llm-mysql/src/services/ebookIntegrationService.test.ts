...
describe('EbookIntegrationService', () => {
        ...
        successfullyProcessed: [
          {
            ...
            author: '測試作者1',
            summary: '這是測試摘要1',
            downloadUrl: 'https://example.com/book1.pdf?openExternalBrowser=1',
          ...
          },
          {
            title: '測試書籍2',
            summary: '這是測試摘要2',
            downloadUrl: 'https://example.com/book2.pdf?openExternalBrowser=1',
    ...
    test('should emit events when files are processed', (done) => {
      const testData = {
        ...
        successfullyProcessed: [{
          title: '測試書籍',
          summary: '測試摘要',
          downloadUrl: 'https://example.com/test.pdf?openExternalBrowser=1',
  ...
  describe('addExternalBrowserParam', () => {
    it('should add openExternalBrowser=1 parameter to PDF URLs', () => {
      
      // 使用 any 類型來訪問私有方法進行測試
      const addExternalBrowserParam = (service as any).addExternalBrowserParam.bind(service);
      
      const testCases = [
        {
          input: 'https://www2.budaedu.org/dharma-data/book-efile/CH826-21-01-001.pdf',
          expected: 'https://www2.budaedu.org/dharma-data/book-efile/CH826-21-01-001.pdf?openExternalBrowser=1'
        },
        {
          input: 'https://example.com/book.pdf?existing=param',
          expected: 'https://example.com/book.pdf?existing=param&openExternalBrowser=1'
        },
        {
          input: 'https://example.com/book.pdf?openExternalBrowser=1',
          expected: 'https://example.com/book.pdf?openExternalBrowser=1'
        },
        {
          input: '',
          expected: ''
      ...
      testCases.forEach(({ input, expected }) => {
        const result = addExternalBrowserParam(input);
    ...
    it('should handle invalid URLs gracefully', () => {
      ...
      const result = addExternalBrowserParam(invalidUrl);
