...
class NewBookSummaryApp:
    ...
    def on_closing(self):
        ...
        try:
            # Reload configuration from file before saving to avoid overwriting updates
            # made by MainProcessor (e.g., baseline_book_title updates)
            if self.config_manager:
                self.config_manager.load_config()
                # Update UI with latest values from config file
                self.baseline_title_var.set(self.config_manager.get('baseline_book_title', ''))
            
            # Save configuration before closing (preserves any UI changes)
            self.save_config()
    ...
    def _update_ui_status(self, message: str):
        ...
        if hasattr(self, 'main_processor') and self.main_processor:
            if not self.main_processor.is_running:
                # Processing has finished, reload configuration to get updated baseline_book_title
                if self.config_manager:
                    self.config_manager.load_config()
                    self.baseline_title_var.set(self.config_manager.get('baseline_book_title', ''))
                    self.logger.info(f"已重新載入配置，baseline_book_title: {self.config_manager.get('baseline_book_title', '')}")
